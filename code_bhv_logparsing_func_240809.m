...
log_path_out = '../data/0902/total/data_bhv_log_table';
plot_path_out = '../data/0902/total/data_bhv_plot';
curve_path_out = '../data/0902/total/data_learning_curve';



path = {log_path_in,log_path_out,plot_path_out,curve_path_out};
if create_dir ==1
    for i=1:length(path)
        mkdir(path{i});
    end
...
sbj_info_file = readtable([log_path_out '\participants_total.tsv'],'FileType','text');
...
for sbj_i = 1:n_sbj
    c_sbj=sprintf('sub-%.2d',sbj_i);
    disp(['Current subject: ', c_sbj]);

    [all_sbj_events_temp,num_sbj_events_temp,fig,sbj_info_file_temp] = func_bhv_logparsing_240703(path,sbj_i,is_save_output,is_open_plot,create_dir,sbj_info_file_temp);
...
if is_save_output == 1
    ...
    save(fullfile(path{2},'total','all_sbj_events.mat') ,"all_sbj_events");
...
if create_bhv_plots ==1
    ...
    [sbj_perform, sbj_info, figs] = func_bhv_performance_plots(n_sbj, is_save_output, all_sbj_events, sbj_info_file_temp,plot_path_out);
