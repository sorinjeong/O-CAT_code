...
index = 1:size(good_data, 1);
mean_vals = mean(good_data, 2);
std_vals = std(good_data, 0, 2);

figure;
hold on;
for i = 1:size(good_data, 2)
    plot(index, good_data(:, i), 'Color', [0.8 0.8 0.8], 'LineWidth', 1); 
end

fill([index, fliplr(index)], [mean_vals + std_vals; flipud(mean_vals - std_vals)], 'r', 'FaceAlpha', 0.2, 'EdgeColor', 'none');
plot(index, mean_vals, 'r-', 'LineWidth', 2);

xlabel('Index');
ylabel('Performance');
title('Accuracy Plot for Each Subject with Shadow for Error and Mean Value');
